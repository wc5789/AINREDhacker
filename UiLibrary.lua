...
-- 安全的拖动函数（支持PC和移动端）
local function setupDrag(dragObject, targetObject)
...
function UILibrary.Load(GUITitle)
    ...
    NewInstance.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
    ...
    -- ===== 修复：使用新的拖动系统 =====
    setupDrag(TitleBar, ContainerFrame)
    
    -- 最小化按钮
    MinimiseButton.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 or 
           input.UserInputType == Enum.UserInputType.Touch then
    ...
    function TabLibrary.AddPage(PageTitle, SearchBarIncluded)
        ...
        local PageContainer = RoundBox(5, true)  -- 改为可点击
        ...
        -- 修复：使用 InputBegan 而不是 MouseButton1Down
        PageContainer.InputBegan:Connect(function(input)
            if input.UserInputType == Enum.UserInputType.MouseButton1 or 
               input.UserInputType == Enum.UserInputType.Touch then
                spawn(function()
        ...
        if SearchBarIncluded then
            ...
            local SearchIconImage = SearchIcon()
            SearchIconImage.Parent = SearchBarContainer
        ...
        function PageLibrary.AddButton(Text, Callback, Parent, Underline)
            ...
            local ButtonForeground = RoundBox(5, true)  -- 改为可点击
            ...
            -- 修复：使用 InputBegan
            ButtonForeground.InputBegan:Connect(function(input)
                if input.UserInputType == Enum.UserInputType.MouseButton1 or 
                   input.UserInputType == Enum.UserInputType.Touch then
                    Callback()
        ...
        function PageLibrary.AddDropdown(Text, ConfigurationArray, Callback)
            ...
            local DropdownForeground = RoundBox(5, true)  -- 改为可点击
            ...
            -- 修复：使用 InputBegan
            DropdownExpander.InputBegan:Connect(function(input)
                if input.UserInputType == Enum.UserInputType.MouseButton1 or 
                   input.UserInputType == Enum.UserInputType.Touch then
        ...
        function PageLibrary.AddColourPicker(Text, DefaultColour, Callback)
            ...
            local PickerRightSide = RoundBox(5, true)  -- 改为可点击
            ...
            local GreenPicker = PageLibrary.AddSlider("G", {Min = 0, Max = 255, Def = ColourTracker.Value.G * 255}, function(Value)
                ColourTracker.Value = Color3.fromRGB(ColourTracker.Value.R * 255, Value, ColourTracker.Value.B * 255)
                Callback(ColourTracker.Value)
            end, PickerFrame)
            
            local BluePicker = PageLibrary.AddSlider("B", {Min = 0, Max = 255, Def = ColourTracker.Value.B * 255}, function(Value)
            ...
            -- 修复：使用 InputBegan
            PickerRightSide.InputBegan:Connect(function(input)
                if input.UserInputType == Enum.UserInputType.MouseButton1 or 
                   input.UserInputType == Enum.UserInputType.Touch then
        ...
        function PageLibrary.AddSlider(Text, ConfigurationDictionary, Callback, Parent)
            ...
            local SliderForeground = RoundBox(5, true)  -- 改为可点击
            ...
            -- 修复：使用 InputBegan 处理滑块拖动
            SliderForeground.InputBegan:Connect(function(input)
                if input.UserInputType == Enum.UserInputType.MouseButton1 or 
                   input.UserInputType == Enum.UserInputType.Touch then
                    Tween(SliderFill, {ImageTransparency = 0.5})
                    local X, Y, XScale, YScale = GetXY(SliderForeground)
                    ...
                    SliderKill = UserInputService.InputEnded:Connect(function(UserInput)
                        if UserInput.UserInputType == Enum.UserInputType.MouseButton1 or 
                           UserInput.UserInputType == Enum.UserInputType.Touch then
        ...
        function PageLibrary.AddToggle(Text, Default, Callback)
            ...
            local ToggleLeftSide = RoundBox(5, true)  -- 改为可点击
            ...
            -- 修复：使用 InputBegan
            ToggleLeftSide.InputBegan:Connect(function(input)
                if input.UserInputType == Enum.UserInputType.MouseButton1 or 
                   input.UserInputType == Enum.UserInputType.Touch then
